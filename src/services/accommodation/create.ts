...
export class AccommodationCreateService{
    execute = async({title, description, arrival, departure, address, user_id, event_id} : AccommodationRequest) : Promise<Accommodation | Error> => {
        ...
        const accommodation = repository.create({title, description, arrival, departure, address, user_id, event_id});
