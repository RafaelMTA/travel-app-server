...
export class AccommodationCreateService{
    execute = async({name, description, arrival, departure, user_id, event_id} : AccommodationRequest) : Promise<Accommodation | Error> => {
        ...
        const accommodation = repository.create({name, description, arrival, departure, user_id, event_id});
